...
| [Gemini Local](/adapters/gemini-local) | `gemini_local` | Gemini CLI를 로컬에서 실행 |
| [Command Code Local](/adapters/commandcode-local) | `commandcode_local` | Command Code CLI를 로컬에서 실행 |
| [Cursor Local](/adapters/cursor-local) | `cursor` | Cursor CLI를 로컬에서 실행 |
| [Pi Local](/adapters/pi-local) | `pi_local` | Pi CLI를 로컬에서 실행 |
| [Antigravity Local](/adapters/antigravity-local) | `antigravity_local` | Antigravity CLI를 로컬에서 실행 |
| [Hermes Local](/adapters/hermes-local) | `hermes_local` | Hermes를 로컬에서 실행 |
| OpenCode Local | `opencode_local` | OpenCode CLI를 로컬에서 실행(멀티 프로바이더 `provider/model`) |
| [OpenClaw Gateway](/adapters/openclaw-gateway) | `openclaw_gateway` | wake 페이로드를 OpenClaw webhook으로 전송 |
| [Process](/adapters/process) | `process` | 임의의 셸 명령 실행 |
| [HTTP](/adapters/http) | `http` | 외부 에이전트로 webhook 전송 |
...
- **커스텀이 필요한가요?** [자체 어댑터 만들기](/adapters/creating-an-adapter) 문서를 참고하세요.
