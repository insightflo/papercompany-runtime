...
How to get OpenClaw running in a Docker container for local development and testing the papercompany OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

papercompany includes an end-to-end join smoke harness:
...
- probes and prints a papercompany host URL that is reachable from inside OpenClaw Docker
...
If your papercompany deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host papercompany server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed papercompany URL (typically `http://host.docker.internal:3100`).
- If papercompany rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart papercompany and rerun the smoke script.
