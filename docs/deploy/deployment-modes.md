...
papercompany supports two runtime modes with different security profiles.
...
When migrating from `local_trusted` to `authenticated`, papercompany emits a one-time claim URL at startup:
