...
Audience: Operators setting up and running agents in papercompany

## 1. What this system does

Agents in papercompany do not run continuously.  
...
papercompany stores resumable session state per `(agent, taskKey, adapterType)`.
...
papercompany pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. papercompany surfaces this as a warning in environment tests, not a hard error.
