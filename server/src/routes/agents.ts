...
export function agentRoutes(db: Db) {
  const DEFAULT_INSTRUCTIONS_PATH_KEYS: Record<string, string> = {
    ...
    commandcode_local: "instructionsFilePath",
  };
  ...
  function mergeDefaultParentIssueIdIntoMetadata(input: Record<string, unknown>) {
    if (!Object.prototype.hasOwnProperty.call(input, "defaultParentIssueId")) return input;
    const next = { ...input };
    const defaultParentIssueId =
      typeof next.defaultParentIssueId === "string" && next.defaultParentIssueId.trim().length > 0
        ? next.defaultParentIssueId.trim()
        : null;
    const metadata =
      typeof next.metadata === "object" && next.metadata !== null && !Array.isArray(next.metadata)
        ? { ...(next.metadata as Record<string, unknown>) }
        : {};
    metadata.defaultParentIssueId = defaultParentIssueId;
    next.metadata = metadata;
    delete next.defaultParentIssueId;
    return next;
  }
  ...
  router.get("/companies/:companyId/adapters/:type/models", async (req, res) => {
    ...
    const provider =
      typeof req.query.provider === "string" ? req.query.provider.trim() : undefined;
    const models = await listAdapterModels(type, provider);
    res.json(models);
  });
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    ...
    const normalizedBody = mergeDefaultParentIssueIdIntoMetadata(req.body as Record<string, unknown>);
    const {
      desiredSkills: requestedDesiredSkills,
      ...createInput
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    const patchData = mergeDefaultParentIssueIdIntoMetadata(req.body as Record<string, unknown>);
