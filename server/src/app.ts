...
import { checkCoreWorkflowToolsAvailable, executeCoreWorkflowTool, resolveWorkflowRunStepEnv } from "./services/workflow/core-tool-executor.js";
...
export async function createApp(
  ...
  const missionOwnerSupervisionMonitor = createMissionOwnerSupervisionMonitor(db, {
    ...
    onPlanSubmissionMissing: ({ mission, planIssueId, targetAgentId, idempotencyKey, wakeCommentId }) => heartbeat.wakeup(targetAgentId, {
      source: "assignment",
      triggerDetail: "system",
    ...
    onMissionTerminalRunCloseoutWakeRequested: ({ mission, run, planUnitKey, oversightIssueId, idempotencyKey }) => heartbeat.wakeup(mission.ownerAgentId, {
      source: "assignment",
      triggerDetail: "system",
      reason: "mission_terminal_run_closeout",
      idempotencyKey,
      payload: {
        ...(oversightIssueId ? { issueId: oversightIssueId } : {}),
        missionId: mission.id,
        mutation: "mission_terminal_run_closeout",
        workflowRunId: run.id,
        runStatus: run.status,
        planUnitKey,
        hint: "Execution unit reached a terminal status while the mission is still active. Reconcile the mission: record owner judgement, sync plan unit status, and close out the oversight issue or take an explicit owner decision.",
      },
      requestedByActorType: "system",
      requestedByActorId: "mission-owner-supervision-monitor",
      contextSnapshot: {
        missionId: mission.id,
        source: "mission_supervision_closeout",
        workflowRunId: run.id,
        runStatus: run.status,
        planUnitKey,
      },
    }),
    onOwnerDecisionDispatchStalledWakeRequested: ({ mission, ownerActionIssue, sourceIssue, dispatchStatus, idempotencyKey }) => heartbeat.wakeup(mission.ownerAgentId, {
      source: "assignment",
      triggerDetail: "system",
      reason: "mission_owner_decision_dispatch_stalled",
      idempotencyKey,
      payload: {
        issueId: ownerActionIssue.id,
        missionId: mission.id,
        mutation: "mission_owner_decision_dispatch_stalled",
        sourceIssueId: sourceIssue.id,
        dispatchStatus,
        hint: "Your retry_source_issue decision was recorded but its dispatch keeps failing. Re-submit the decision with a dispatchable target (assignee/native link) or choose replan/escalate.",
      },
      requestedByActorType: "system",
      requestedByActorId: "mission-owner-supervision-monitor",
      contextSnapshot: {
        issueId: ownerActionIssue.id,
        missionId: mission.id,
        source: "mission_owner_decision_dispatch_stalled",
        sourceIssueId: sourceIssue.id,
        dispatchStatus,
      },
    }),
