...
export function buildMissionPlanningDescription(input: MissionPlanningDescriptionInput): string {
  ...
  return [
    ...
    "- A tool grant establishes eligibility, not semantic fit. Before adding a toolName, inspect the matching planning dossier tool entry's description and inputSchema, then compare them with the unit's upstream expectedOutput, acceptanceCriteria, and evidenceRequired. Never select a tool from its name/displayName alone.",
    "- Treat planningMetadata.acceptedInputKinds as a hard compatibility constraint when present. If the upstream artifact kind is incompatible (for example, an HTML page for a Markdown-only validator), reject the plan or select a compatible tool; do not rely on the Agent to adapt the payload at execution time.",
