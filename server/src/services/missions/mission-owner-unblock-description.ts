...
export function buildMainExecutorBrief(input: {
  ...
  return [
    ...
    "- If a recorded pattern card motivates a bounded company skill patch (one section add/replace/delete), adopt it through the self-improvement loop so provenance is kept: submit the candidate with evidenceSource citing the card ({type: \"knowledge_pattern\", id: <card id>} or \"knowledge_pattern:<id>\"), collect the peer gate verdict (PASS/FAIL), then POST /api/companies/{companyId}/self-improvement-adoptions/apply (use /dry-run first to inspect the plan). Avoid hand-editing skill markdown outside this loop — direct edits leave no adoption ledger.",
