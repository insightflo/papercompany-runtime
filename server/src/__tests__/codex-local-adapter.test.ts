  ...
  it("parses command execution and file changes", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            ...
            command: '/bin/zsh -lc "curl -H \\"Authorization: Bearer token-123\\" http://localhost:3100/api"',
            status: "in_progress",
          },
        }),
        ts,
    ...
    ).toEqual([
      {
        ...
        input: {
          id: "item_2",
          command: '/bin/zsh -lc "curl -H \\"Authorization: Bearer ***REDACTED***\\" http://localhost:3100/api"',
        },
      },
    ]);
      ...
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            ...
            type: "command_execution",
            command: '/bin/zsh -lc "curl -H \\"Authorization: Bearer token-123\\" http://localhost:3100/api"',
            aggregated_output: 'Authorization: Bearer token-123\nPAPERCLIP_API_KEY=token-123\n',
            exit_code: 0,
            status: "completed",
          },
        }),
  ...
  it("prints lifecycle, command execution, file change, and error events", () => {
    ...
    try {
      ...
      expect(lines).toEqual(expect.arrayContaining([
        ...
        '/bin/zsh -lc "curl -H \\"Authorization: Bearer ***REDACTED***\\" http://localhost:3100/api"',
        "Authorization: Bearer ***REDACTED***",
      ...
      expect(
        lines.some((line) =>
          line.includes("tool_result: command_execution") &&
          line.includes("Authorization: Bearer ***REDACTED***") &&
          line.includes("status=completed exit_code=0"),
        ),
      ).toBe(true);
