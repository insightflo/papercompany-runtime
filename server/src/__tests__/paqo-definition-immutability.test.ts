...
// [T7 격리] helpers/paqo-immutability 의 recordLatest 가 PLAN-QA manifest 를 StorageService 에
// 기록하므로 기본 인스턴스 storage(~/.paperclip) 대신 suite 전용 임시 디렉터리로 돌린다.
let planQaStorageRoot: string | null = null;
beforeAll(async () => {
  planQaStorageRoot = await mkdtemp(path.join(os.tmpdir(), "paperclip-paqo-imm-storage-"));
