...
export const sessionCodec: AdapterSessionCodec = {
  deserialize(raw: unknown) {
    ...
    const sessionId = readNonEmptyString(record.sessionId) ?? readNonEmptyString(record.conversationId);
  ...
  serialize(params: Record<string, unknown> | null) {
    ...
    const sessionId = readNonEmptyString(params.sessionId) ?? readNonEmptyString(params.conversationId);
    if (!sessionId) return null;
  ...
  getDisplayId(params: Record<string, unknown> | null) {
    if (!params) return null;
    return readNonEmptyString(params.sessionId) ?? readNonEmptyString(params.conversationId);
  },
};
