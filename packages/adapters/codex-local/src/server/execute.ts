...
import { sanitizeCodexLogLine, sanitizeCodexLogStream } from "../redaction.js";
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    if (stdoutBuffer) {
      await onLog("stdout", sanitizeCodexLogLine(stdoutBuffer, sensitiveValues));
    }
    const cleanedStderr = stripCodexRolloutNoise(proc.stderr);
    const sanitizedStdout = sanitizeCodexLogStream(proc.stdout, sensitiveValues);
    return {
      proc: {
        ...proc,
      ...
      parsed: parseCodexJsonl(sanitizedStdout),
    };
  };
  ...
  const toResult = (
    ...
    const isSuccess = (attempt.proc.exitCode ?? 0) === 0;
    const codexAuthErrorCode = isSuccess ? null : resolveCodexAuthErrorCode(fallbackErrorMessage);
