...
import { sanitizeCodexLogLine } from "../redaction.js";
...
export function parseCodexStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const sanitizedLine = sanitizeCodexLogLine(line);
  const parsed = asRecord(safeJsonParse(sanitizedLine));
  if (!parsed) {
    return [{ kind: "stdout", ts, text: sanitizedLine }];
  }
  ...
  if (type === "error") {
    ...
    return [{ kind: "stderr", ts, text: message || sanitizedLine }];
  }
