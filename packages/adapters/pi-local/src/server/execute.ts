...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[paperclip] Removed maintainer-only Pi skill "${skillName}" from ${skillsHome}\n`,
    );
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const sessionsDir = resolvePiSessionsDir(config);
  const skillsDir = resolvePiSkillsDir(config);
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      systemPromptExtension =
        ...
        `Follow the Paperclip heartbeat procedure exactly. For assignments, use GET /api/agents/me/inbox-lite first. ` +
        `Fall back only to the company issues endpoint filtered by assigneeAgentId with statuses todo,in_progress,blocked. ` +
        `Do not improvise alternate issue query parameters such as status=open, assigneeId, or agentId. ` +
        `If no assignments are returned, exit the heartbeat.`;
      await onLog(
        "stdout",
  ...
  const toResult = (
    ...
    const resolvedSessionId = clearSessionOnMissingSession ? null : resolvedSessionPath;
