...
import { syncGeminiSkills } from "./skills.js";

async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await mkdtemp(path.join(tmpdir(), "papercompany-gemini-skills-"));
  try {
    return await fn(dir);
  } finally {
...
describe("syncGeminiSkills", () => {
  it("removes stale Papercompany-managed materialized skill directories when deselected", async () => {
    await withTempDir(async (dir) => {
      ...
      const skillsHome = path.join(home, ".gemini", "skills");
