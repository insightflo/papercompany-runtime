...
import { createScriptSearchAdapter } from "../dist/adapters/script-search.js";
...
async function writeSearchFixtureScript(body) {
  const dir = await mkdtemp(path.join(tmpdir(), "research-workbench-script-"));
  const scriptPath = path.join(dir, "search.mjs");
...
test("script adapter runs a local command with JSON stdin and parses JSON stdout", async () => {
  const scriptPath = await writeSearchFixtureScript(`
    let input = "";
    ...
    process.stdin.on("end", () => {
      ...
      process.stdout.write(JSON.stringify({
        query: request.query,
        results: [
          { title: "Script Result", url: "https://example.com/script", snippet: "from script", source: "script" }
        ],
        engine: { patchVersion: "script-fixture" }
      }));
    });
  `);

  const adapter = createScriptSearchAdapter({
    command: `${process.execPath} ${scriptPath}`,
    timeoutMs: 5000,
  });

  const result = await adapter.search({ query: "script query", maxResults: 3 });

  assert.equal(result.ok, true);
  assert.equal(result.engine.name, "script");
  assert.equal(result.engine.patchVersion, "script-fixture");
  assert.equal(result.results.length, 1);
  assert.equal(result.results[0].title, "Script Result");
});
...
test("worker handleResearchSearch returns evidence bundle via script backend", async () => {
  const scriptPath = await writeSearchFixtureScript(`
    let input = "";
    process.stdin.setEncoding("utf8");
    process.stdin.on("data", (chunk) => { input += chunk; });
    process.stdin.on("end", () => {
      ...
      process.stdout.write(JSON.stringify({
        query: request.query,
        results: [
          { title: "Script Worker Result", url: "https://example.com/worker", snippet: "worker script", source: "script" }
        ],
        engine: { patchVersion: "worker-script-fixture" }
      }));
    });
  `);
  ...
  const result = await harness.executeTool("research-search", {
    query: "papercompany script research",
  });

  assert.ok(!result.error, `Expected no error, got: ${result.error}`);
  assert.equal(result.data.query, "papercompany script research");
  assert.equal(result.data.sources.length, 1);
  assert.equal(result.data.rawEngine.name, "script");
  assert.equal(result.data.rawEngine.patchVersion, "worker-script-fixture");
