...
//   시크릿/상태/복사/오류 상태는 초기화된다. [상태 게이트] 알 수 없는 상태(로딩, 404 아님 오류)
//   에서는 변이 버튼을 렌더하지 않는다 — 알 수 없는 상태의 POST 는 기존 키를 회전시킨다(서버
//   upsert 계약). 실제 404 만 미구성으로 승격해 등록을 허용한다. [B2] 표시 URL 은 실제 공개
...
//   경로가 아니다. 응답 산문은 권위가 아니며 상태 구분은 HTTP 404 코드뿐이다.
