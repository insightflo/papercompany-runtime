...
// [bounded correction] 웹훅 패널 상태 머신 회귀 — 알 수 없는 상태(로딩/404 아님 오류)에서의
// 변이 금지, 실제 404 만의 미구성 승격, enabled true/false 분기를 순수 derive 함수로 검증한다.
...
function derive(over: { loading?: boolean; error?: string | null; errorStatus?: number | null; status?: typeof ENABLED | null } = {}) {
...
describe("deriveWebhookPanelState — unknown-status mutation gating", () => {
  ...
  it("real 404 promotes to unconfigured — the register path stays available (GET404 unconfigured case)", () => {
    const view = derive({ error: "Workflow webhook is not configured", errorStatus: 404 });
    expect(view.phase).toBe("unconfigured");
