...
1. Start papercompany in auth mode.
```bash
cd <papercompany-runtime-repo-root>
pnpm dev --tailscale-auth
```
Then verify:
```bash
curl -sS http://127.0.0.1:3200/api/health | jq
```
...
3. In papercompany UI, go to `http://127.0.0.1:3200/CLA/company/settings`.
...
5. Approve the join request in papercompany UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from papercompany invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in papercompany with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in papercompany UI that new issue exists.
