...
This spec defines how papercompany actually runs agents while staying runtime-agnostic.
...
1. papercompany is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, papercompany manages lifecycle, and on exit papercompany parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared papercompany skill text block)
