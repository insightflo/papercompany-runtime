...
- Builtin and HTTP tools may explicitly opt into `adapterConfig.progress` version 1. Only increasing machine-produced counters on declared ordered stages extend the idle deadline; the DB freezes the policy and total-duration deadline per execution. Company/execution capability, generation/retry/iteration scope, terminal-state checks, and transactional activity entries guard advances. Progress never replaces final-result validation or workflow completion authority.
- Local progress uses bounded newline JSON on fd3, separate from stdout/stderr. HTTP progress uses a dedicated capability header and a trusted HTTPS callback base; redirects are rejected. Without progress, HTTP fixed deadlines support values above five minutes through 2147483647ms and cover response bodies. MCP/plugin behavior is unchanged; synchronous condition tools explicitly disable progress and retain 30 seconds.
- See [Tool progress runbook and executable examples](./runbooks/tool-progress.md) for event/coalescing rules, callback errors, DB diagnostics, rollback, and remote/descendant cancellation limits.
