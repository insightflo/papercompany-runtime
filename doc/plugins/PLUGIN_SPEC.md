# papercompany Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for papercompany's plugin and extension architecture.
...
Compatibility note: package and SDK examples may still use `paperclipai` / `@paperclipai/*` names. Those are current compatibility interfaces; the product and plugin model described here are papercompany.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main papercompany app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary papercompany HTTP APIs directly.
...
papercompany plugin design is based on the following assumptions:

1. papercompany is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core papercompany business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by papercompany core.
...
2. Let plugins add major capabilities without editing papercompany core.
...
The single papercompany deployment an operator installs and controls.

### 5.2 Company

A first-class papercompany business object inside the instance.
...
A trusted in-process extension loaded directly by papercompany core.
...
An installable instance-wide extension package loaded through the papercompany plugin runtime.
...
papercompany has two extension classes.
...
papercompany already has a concrete workspace model:
...
Plugins live under the papercompany instance directory.
...
papercompany should add CLI commands:
...
Plugins may contribute tools that papercompany agents can use during runs.
...
- papercompany server starts one worker process per installed plugin
...
Receives one typed papercompany domain event.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the papercompany secret provider system rather than a plain text input
...
1. Core papercompany data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing papercompany objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual papercompany product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if papercompany later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing papercompany secret provider system.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the papercompany server. This is a normative requirement, not optional.
...
For developing a plugin against a running papercompany instance:
...
papercompany should not implement a generic in-process hook bag modeled directly after local coding tools.

papercompany should implement:
...
That is the complete target design for the papercompany plugin system.
