# papercompany Specification

Target specification for the papercompany control plane. Living document — updated incrementally during spec interviews.

Lineage: papercompany is derived from the original [Paperclip](https://github.com/paperclipai/paperclip) control-plane codebase. This long-horizon spec describes the papercompany direction; compatibility names such as `paperclipai`, `@paperclipai/*`, `PAPERCLIP_*`, and `~/.paperclip` remain implementation details where they are still the active runtime contract.

## What Changed From Paperclip

papercompany keeps Paperclip's agent-control-plane foundation, then expands it toward running agent companies:

- company-scoped missions, goals, org structure, budgets, approvals, work products, and outcomes are treated as one operating loop
- server-native workflow/DAG execution owns workflow definitions, runs, step runs, resume behavior, and downstream work creation
- mission completion depends on evidence, artifacts, reports, files, links, or verification signals, not only agent self-report
- failures are surfaced as explicit recovery states so the board can retry, escalate, pause, or inspect instead of losing the trail
- cross-company mission delegation can hand work to another company while preserving source-side tracking and copied completion artifacts
- the board surface is designed around operations: missions, scheduler state, approvals, channels, costs, exceptions, workflow state, and output review
- local development supports isolated worktree instances with separate ports, embedded databases, config, and branding
- plugins, service-request bridges, research workbench adapters, tool registry integration, and company skills are extension seams around the core company workflow

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One papercompany instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the papercompany protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

papercompany doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### papercompany Protocol (What papercompany Knows)

At the protocol level, papercompany tracks:
...
- **Fat payload** — papercompany bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to papercompany.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls papercompany's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

As the runtime gets thinner and more server-governed, papercompany now attaches a server-computed step input manifest describing the adapter-visible inputs for the current heartbeat. In the current slice this manifest is primarily descriptive, and it also blocks a narrow first set of explicit repo/workspace-wide scan instructions before adapter execution when broad scans are disallowed.

#### Minimum Contract

The minimum requirement to be a papercompany agent: **be callable.** That's it. papercompany can invoke you via command or webhook. No requirement to report back — papercompany infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, papercompany provides progressively richer integration:

1. **Callable** (minimum) — papercompany can start you. That's the only contract.
...
papercompany ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **papercompany skill** (a Claude Code skill for interacting with the papercompany API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the papercompany SKILL.md which is recommended for all agents. papercompany the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. papercompany defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how papercompany invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets papercompany check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the papercompany REST API.

### What papercompany Controls
...
### What papercompany Does NOT Control
...
- Health monitoring — how does papercompany distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of papercompany. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to papercompany. Costs are tracked at every level:
...
papercompany ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **papercompany Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### papercompany Skill (SKILL.md)

A skill definition that teaches agents how to interact with papercompany. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my papercompany instance."

#### Agent Authentication

When a user creates an Agent, papercompany generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. papercompany generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), papercompany sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the papercompany SKILL, but sometimes it's unavoidable.
...
papercompany does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. papercompany tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, papercompany does **not** auto-reassign or auto-release the task. Instead:

- papercompany surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- papercompany does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: papercompany reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core papercompany system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
**Full loop with one adapter.** V1 must demonstrate the complete papercompany cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with papercompany skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **papercompany skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the papercompany core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things papercompany explicitly does **not** do:

- **Not an Agent runtime** — papercompany orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. papercompany is the control plane, not the execution plane.
